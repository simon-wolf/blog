---
layout: page
title: 2026 Goals
permalink: /goals_2026/
---

* toc placeholder
{:toc}

# Weight Loss

## Reach Target Weight

* Goal: Reach my target weight of around 70 kg.
* Status: Started 2026 at 79.8 kg.

# Running

## 5k Pace

* Goal: Run 5k in under 30 minutes.
* Status: Achieved on 24 May 2025 during the Taunton Trail 10k.
* PB is 28:57 on 4 April 2026.

## 10k Pace

* Goal: Run 10k in under 60 minutes.
* Status: PB is 1:02:50 on 24 May 2026.

## Regularly Attend parkruns

* Goal: Hit the 10 milestone.
* Status: In progress.

| Date | Location | Time |
| --- | --- | --- |
| 2025-07-19 | Shepton Mallet | 35:35 |
| 2025-07-26 | Shepton Mallet | 35:27 |
| 2025-10-18 | Shepton Mallet | 32:13 |
| 2026-01-17 | Shepton Mallet | 33:14 |
| 2026-02-14 | Shepton Mallet | 32:00 |
| 2026-04-04 | Shepton Mallet | 31:03 |
| 2026-04-11 | Shepton Mallet | 31:51 |
| 2026-04-25 | Shepton Mallet | 30:52 |

## Volunteer At parkruns

* Goal: Volunteer at 5 parkruns.
* Status: Complete.

| Date | Location | Role |
| --- | --- | --- |
| 2026-01-31 | Shepton Mallet | Token Support |
| 2026-02-07 | Shepton Mallet | Token Support |
| 2026-02-21 | Shepton Mallet | Course Marshal |
| 2026-04-18 | Shepton Mallet | Course Marshal |
| 2026-05-30 | Shepton Mallet | Course Marshal |

## 10K Races

* Goal: Run four 10k races.
* Status: Pending.

| Date | Location | Time |
| --- | --- | --- |
| 2026-05-24 | Taunton Trail 10K | Pending |
| 2026-06-07 | Chew Valley 10K | Pending |
| 2026-08-16 | Frome 10K | Pending |

## 5K Races

* Goal: Run four 5k races.
* Status: Pending.

## Improve Slow Running

* Goal: Improve running in Zone 2.
* Status: Starting 2026 around 9 minutes per kilometer

# Crossfit

## Skipping

* Goal: 50 single skips unbroken.
* Status: Pending.

## Track Core Statistics

* Goal: Track values such as maximum deadlifts, kettlebell swing weights, etc.
* Status: Pending.

## Attend Additional Sessions

* Goal: In addition to two regular weekly sessions, join additional ones such as weight lifting.
* Status: Pending.

# Zwift

## Improve FTP

* Goal: Reach an FTP of 250.
* Status: Pending

## Improve Alpe du Zwift Time

* Goal: Ride [Alpe du Zwift](https://zwiftinsider.com/segment/alpe-du-zwift/) in under 75 minutes.
* Status: Pending.

# Flexibility

* Goal: Find a yoga or Pilates class to start improving my flexibility.
* Status: Pending.

# Previous Years

* [2025]({% link goals_2025.md %})

# Changelog

* *31 December 2026:* Added the 2026 goals page.

